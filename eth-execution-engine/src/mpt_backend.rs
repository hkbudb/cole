...
impl<'a> BackendWriteTrait for MPTExecutorBackend<'a> {
    ...
    fn batch_write(&mut self, states: BTreeMap<AddrKey, StateValue>, _: u32) {
        let transformed_states = states.into_iter().map(|(k, v)| {
            (k, v)
        }).collect();
    ...
    fn mpt_backend_in_disk() {
        ...
        for (k, v) in states {
            ...
            for version in 1..=latest_version {
                ...
                let b = verify_with_addr_key(&k, read_v, backend.states.get_root_with_version(version), &p);
                let elapse = start.elapsed().as_nanos();
                search_prove += elapse;
                if b == false {
                    println!("false");
